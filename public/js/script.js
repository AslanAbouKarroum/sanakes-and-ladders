    ...
    this.play = ()=>{  // called when playing with 2 dice and 2 players
        ...
        if(this.turn==1){  // player 1 turn
            ...
            if(this.player2place!=this.player1place){   
                if(element) element.style['background-color'] = null;  // to turn the background off before leaving the palce
            }else{
                if(element) element.style['background-color'] = 'blue'; // to turn the background by the opposit color before leaving (that was a glitch caused by the comming to the occupied place abd leaving it on the next turn before the opposit play(2 dice equal case))
            }
            ...
            if(element)element.style['background-color'] = 'red';        // coloring the newly occupied place
